...
import com.keene.mepbm.pdfparser.reader.METurnReaderData;
...
public class PDFReaderData extends METurnFileData {
    ...
    public final Hex parseLocation() {
        ...
        Climate climate = new ClimateImpl();
        terrain.setName(token.substring(token.indexOf(" in ") + 4, token.indexOf("  Climate is")));
        climate.setName(token.substring(token.indexOf("Climate is ") + 11, token.length()));
        Hex hex = new HexImpl();
        hex.setClimate(climate);
        hex.setTerrain(terrain);
        hex.setHex(hexNumber);
        return hex;
    }
